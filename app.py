...
def load_data_from_api():
    ...
    try:
        ...
        # --- CORREÇÃO DE MATRÍCULA (CRUCIAL) ---
        # Remove .0 (ex: 1001.0 -> 1001) e espaços para garantir o match
...
if df is not None and not df.empty:
    ...
    if not df_sal.empty:
        # Limpeza na tabela de salários também
        df_sal['Valor'] = pd.to_numeric(df_sal['Valor'], errors='coerce')
        # Garante que Nível de Cargo não tenha espaços extras
    ...
    # --- TRAVA DE SEGURANÇA (FIX PARA O SEU ERRO) ---
    if df_elegiveis.empty:
        st.warning("⚠️ Atenção: A base de dados ficou vazia após o cruzamento.")
        st.markdown("""
        **Possíveis causas:**
        1. As matrículas na aba 'Funcionário' não batem com a aba 'Performance' (ex: texto vs número).
        2. Os Níveis de Cargo (I, II, III) estão escritos diferentes na tabela de Salários.
        """)
        # Mostra diagnóstico para você arrumar
        st.write("--- Diagnóstico ---")
        st.write(f"Linhas após Merge inicial: {len(df)}")
        if not df.empty:
            st.write("Amostra dos Níveis de Cargo encontrados:", df['Nível de Cargo'].unique())
            st.write("Amostra dos Níveis na Tabela Salarial:", list(df_sal_map.keys()) if 'df_sal_map' in locals() else "Vazia")
        st.stop() # Para o código aqui para não dar erro vermelho
    ...
    # Previne erro se sobraram NaNs
    df_elegiveis[cols_norm] = df_elegiveis[cols_norm].fillna(0)
    
    # Agora é seguro rodar o scaler
            ...
            if not promovidos.empty:
                ...
                for _, row in promovidos.iterrows():
                    ax.text(row['Score_Tecnico']+0.1, row['fit_cultural'], 
                            f"ID {row['matricula']}", fontsize=8, weight='bold')
    ...
    with col_tabela:
        ...
        if not promovidos.empty:
            st.dataframe(
                ...
                .style.format({'Score_Tecnico': '{:.2f}', 'Custo_Aumento': 'R$ {:.2f}'})
